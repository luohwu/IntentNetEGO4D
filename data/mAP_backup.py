...
def make_sequence_dataset_for_eval(mode='train',dataset_name='EGO4D'):
    ...
    for video_id in sorted(clip_ids):
        ...
        args.annos_path='/home/luohwu/euler/dataset/EGO4D/nao_annotations'
...
def compute_topK_result(item,k=5):
    ...
    num_pred=len(item['cls'])-1
    quota=(k)*num_gt
    if item[score_type]==[]:
        item[score_type]=item['scores']
    idx=sorted(range(num_pred),key=lambda k:-item[score_type][k])
    item['ro_bbox']=[item['ro_bbox'][i] for i in idx]
    item[score_type] = [item[score_type][i] for i in idx]
            ...
            if i<quota:
                if label == gt_label:
                    ...
                    if iou>0.5 and len(result_cls)<1:
                        ...
                        result_scores.append(item[score_type][i])
            else:
                result_ro_bbox.append(item['ro_bbox'][i])
                result_cls.append(item['cls'][i])
                result_scores.append(item[score_type][i])
